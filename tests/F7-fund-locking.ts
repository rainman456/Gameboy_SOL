...
import { createInitializeAccountInstruction, mintTo, getAccount, getAssociatedTokenAddressSync, ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, ACCOUNT_SIZE } from "@solana/spl-token";
...
describe("F7: Fund Locking via Partial Refund", () => {
  ...
  it("Locks extra funds after refund in pay-to-spawn", async () => {
    ...
    expect(Number(postRefundVault.amount)).to.equal(5000, "Extra funds locked in vault");
  });
});